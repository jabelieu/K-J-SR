...
function default_objective(tree, dataset::Dataset{T,L}, options)::L where {T,L}
    ...
    mrp = dataset.X[1,:]
    pc_pred = mrp .- prediction
    pc_targ = mrp .- dataset.y
    ...
    #     new_z = 4*pi*simpsons_integrate(dens_slice.* r .^ 2, h)
    ...
    for i in 0:(numb_nucl - 1)
        ...
        # @assert int_dens_denom != 0 "Predicted denom. is 0!int_denom=$(int_dens_denom)"
        # @assert int_targ_denom != 0 "Target denom. is 0!"
        if int_dens_denom == 0 || int_targ_denom == 0
            # println("WARNING: Division by zero in charge radius calculation.")
...
objective=objective.replace("{{FRPV}}",str(master_rp[0]))
objective=objective.replace("{{LRPV}}",str(master_rp[-1]))
